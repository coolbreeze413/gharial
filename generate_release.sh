...
if [ $# == 1 ] ; then
    ...
    if [ "$1" == "$GHA_MODE" ] ; then
        ...
        echo ""
        echo "[>> GHARIAL <<] GHA MODE specified!"
        echo ""
...
##########################################################################################
# STEP 1 : pull in all the changes from the master branch
##########################################################################################
git pull


##########################################################################################
# STEP 2 : processing and generate artifact
##########################################################################################
set_spinner_snakey() {
...
echo
echo "creating new release artifact: $RELEASE_NAME in $RELEASES_DIR"
echo
...
##########################################################################################
# STEP 4 : extract PAT from git config
##########################################################################################
...
##########################################################################################
# STEP 8 : if STANDALONE MODE (**NOT** GHA MODE) - MERGE PR, CREATE RELEASE
##########################################################################################
if [ "$GHA_MODE_ENABLED" == "$FALSE_VAL" ] ; then
    ...
    ##########################################################################################
    # STEP 8a : merge PR (squash and merge as a single commit)
    ##########################################################################################
    ...
    ##########################################################################################
    # STEP 8b : pull in latest remote and switch to default branch now
    ...
    ##########################################################################################
    # STEP 8c : identify new tag version to use (semver)
    ##########################################################################################
    # ensure to prevent error when no tags available (important in case the script is run with -e)
...
##########################################################################################
# STEP 9 : wait for release to be published
##########################################################################################
TIMEOUT=120
RETRY_TIME=30
CURR_TIME=0
